...
export const SiteHeader: React.FC<SiteHeaderProps> = ({
  ...
  return (
    <header className="fixed top-0 inset-x-0 z-50 bg-white/90 backdrop-blur-md border-b border-gray-200/50 shadow-sm">
          ...
          <nav className="hidden md:flex items-center gap-6 text-sm font-semibold">
            {navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="relative px-3 py-2 text-gray-700 hover:text-brand-blue transition-all duration-200 group"
              >
                {item.label}
            ...
            <div
              className="inline-flex rounded-xl border-2 border-gray-200 overflow-hidden shadow-sm"
              role="group"
